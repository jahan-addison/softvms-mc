...
static short lcdnew(char* pluginType, void** instance, unsigned short mode,
                    short argc, char* argn[], char* argv[], void* saved) {
  int i;
  /*fprintf(stderr, "New(%s, %p, %d, %d, %p, %p, %p)\n", pluginType, instance,
  mode, argc, argn, argv, saved); for(i=0; i<argc; i++) fprintf(stderr,
  "argn[%d]=%s, argv[%d]=%s\n", i, argn[i], i, argv[i]);*/
...
static void free_framedata(struct instance* inst) {
  ...
  for (i = 0; i < inst->lcdi_nframes; i++)
    if (inst->lcdi_framedata[i] != NULL) free(inst->lcdi_framedata[i]);
...
static short destroy(void** instance, void** save) {
  ...
  *save = NULL;
  if (*instance != NULL) {
    ...
    if (i->lcdi_frametime != NULL) free(i->lcdi_frametime);
    if (i->lcdi_framedata) free_framedata(i);
    free(i);
  }
  return 0;
}
...
static Status myXAllocNamedColor(struct instance* i, char* color_name,
  ...
  if ((s = XAllocNamedColor(i->dpy, i->cmap, color_name, screen_def,
                            exact_def)))
    return s;
...
static void draw(struct instance* i) {
  ...
  if (!i->window || !i->complete || !(i->dpy = XOpenDisplay(NULL))) return;

  if ((img = XCreateImage(i->dpy, i->visual, 1, XYBitmap, 0, malloc(bpl * h), w,
                          h, 32, bpl)) == NULL)
    return;
  ...
  if (i->fcntr < i->lcdi_nframes) {
    ...
    if ((tnext.tv_usec += (i->lcdi_frametime[i->fcntr] % 1000) * 1000) >=
        1000000) {
      tnext.tv_sec++;
  ...
  for (;;) {
    ...
    if (i->fcntr < i->lcdi_nframes) {
      ...
      for (y = 0; y < h; y++)
        for (x = 0; x < w; x++) XPutPixel(img, x, y, !!*d++);
    } else
      for (y = 0; y < h; y++)
        for (x = 0; x < w; x++) XPutPixel(img, x, y, 0);

    XPutImage(i->dpy, i->window, gc, img, 0, 0, 0, 0, i->lcdi_width,
              i->lcdi_height);
    ...
    while (QLength(i->dpy) == 0) {
      ...
      if (i->lcdi_nframes > 1) {
        ...
        if (now.tv_sec > tnext.tv_sec ||
          ...
          if (++i->fcntr >= i->lcdi_nframes) i->fcntr = 0;
          tnext.tv_sec += i->lcdi_frametime[i->fcntr] / 1000;
          if ((tnext.tv_usec += (i->lcdi_frametime[i->fcntr] % 1000) * 1000) >=
              1000000) {
            tnext.tv_sec++;
      ...
      if (select(x_fd + 1, &rfds, NULL, NULL, &t) > 0) {
        if (FD_ISSET(x_fd, &rfds)) XPeekEvent(i->dpy, &ev);
      }
    }

    while (QLength(i->dpy) > 0) XNextEvent(i->dpy, &ev);
  }
}
...
static short setwindow(void** instance, void* window) {
  ...
  if (win->win == i->window) return 0;
  i->dpy = info->dpy;
  i->window = win->win;
  i->cmap = info->cmap;
  i->visual = info->vis;
  if (i->complete) start_drawing(i);
  return 0;
}

static short newstream(void** instance, char* type, void* stream,
                       unsigned char seekable, unsigned short* stype) {
  struct instance* i = *instance;
...
static int write(void** instance, void* stream, int offset, int len,
                 void* buffer) {
  int n;
  struct instance* i = *instance;
  unsigned char* hdr = i->hdr;
  unsigned char* b = buffer;
  /*fprintf(stderr, "Write(%p, %p, %d, %d, %p)\n", instance, stream, offset,
   * len, buffer);*/
  if (i->complete) return 0;
  if (len > i->rcnt) len = i->rcnt;
  if (len <= 0) return len;
  switch (i->rstate) {
    case 0:
  ...
  if ((i->rcnt -= len) == 0) {
    ...
    switch (++i->rstate) {
      case 1:
        if (strncmp(hdr, "LCDi", 4)) break;
        i->lcdi_width = hdr[6] + (hdr[7] << 8);
        i->lcdi_height = hdr[8] + (hdr[9] << 8);
        i->lcdi_nframes = hdr[14] + (hdr[15] << 8);
        i->fsz = i->lcdi_width * i->lcdi_height;
        if ((i->lcdi_frametime = calloc(i->lcdi_nframes, sizeof(long))) ==
                NULL ||
            (i->lcdi_framedata =
                 calloc(i->lcdi_nframes, sizeof(unsigned char*))) == NULL)
          break;
      ...
      case 2:
        if (i->fcntr < i->lcdi_nframes) {
          if ((i->lcdi_framedata[i->fcntr] = malloc(i->fsz)) == NULL) break;
          i->rcnt = i->fsz;
        } else {
          i->complete = 1;
          i->fcntr = 0;
          /*fprintf(stderr, "Complete, window = %x\n", i->window);*/
          if (i->window) start_drawing(i);
        }
        break;
    }
  }
...
static struct NPPluginFuncs {
  ...
  short (*newp)(char* pluginType, void** instance, unsigned short mode,
                short argc, char* argn[], char* argv[], void* saved);
  short (*destroy)(void** instance, void** save);
  short (*setwindow)(void** instance, void* window);
  short (*newstream)(void** instance, char* type, void* stream,
                     unsigned char seekable, unsigned short* stype);
  ...
  int (*write)(void** instance, void* stream, int offset, int len,
               void* buffer);
  void (*print)(void** instance, void* platformPrint);
  short (*event)(void** instance, void* event);
  void (*urlnotify)(void** instance, const char* url, short reason,
                    void* notifyData);
  void* javaClass;
} NPPluginFuncs = {sizeof(struct NPPluginFuncs),
                   9,
                   lcdnew,
                   destroy,
                   setwindow,
...
short NP_Initialize(struct NPNetscapeFuncs* nsTable,
                    struct NPPluginFuncs* pluginFuncs) {
  /*fprintf(stderr, "NP_Initialize\n");*/
  if (nsTable == NULL || pluginFuncs == NULL) return 3;
  if (nsTable->version > 255) return 8;
  if (nsTable->size < sizeof(struct NPNetscapeFuncs)) return 3;
  if (pluginFuncs->size < sizeof(struct NPPluginFuncs)) return 3;
