...
int tobcd(int n) { return ((n / 10) << 4) | (n % 10); }
...
void fakeflash(char* filename, int sz) {
  ...
  for (i = strlen(filename); i < 12; i++) dir[4 + i] = ' ';
...
void check_gamesize() {
  ...
  if (fatblk >= 256 || dircnt >= 256 || !dircnt) return;
...
int loadflash(char* filename) {
  ...
  if ((fd = open(filename, O_RDONLY | O_BINARY)) >= 0) {
    ...
    close(fd);
    gamesize = r;
    if (r < 241 * 512) fakeflash(filename, r);
    check_gamesize();
    return 1;
  } else {
    perror(filename);
...
void writemem(int addr, int value) {
  ...
  if (addr >= 0x180) {
    int b = sfr[0x25];
    if (b > 2 || (b == 2 && addr >= 0x186)) return;
    xram[b][addr - 0x180] = value;
    if (lcdon) lcd_updated = 1;
  } else
    switch (addr) {
      case 0x100:
      ...
      case 0x10d:
        if ((value & 1) != (sfr[0x0d] & 1)) {
          ...
          if (hasbios) rom = ((value & 1) ? flash : bios);
        }
        break;
      case 0x10e:
        switch (value & 0xa0) {
        ...
        break;
      case 0x110:
        if (!(value & 0x40)) t0l = sfr[0x13];
        if (!(value & 0x80)) t0h = sfr[0x15];
        break;
      case 0x111:
        t0scale = 256 - value;
        t0base = 0;
        break;
      case 0x113:
        if (!(sfr[0x10] & 0x40)) t0l = value;
        break;
      case 0x115:
        if (!(sfr[0x10] & 0x80)) t0h = value;
        break;
      case 0x118:
        if (!(value & 0x40)) t1l = sfr[0x1b];
        if (!(value & 0x80)) t1h = sfr[0x1d];
        break;
      case 0x11b:
        if (!(sfr[0x18] & 0x40)) t1l = value;
        break;
      case 0x11d:
        if (!(sfr[0x18] & 0x80)) t1h = value;
        break;
      case 0x122:
        if (lcdon) lcd_updated = 1;
        break;
...
int readmem(int addr) {
  int r;
  if (addr < 0x100) return ram[(sfr[0x01] & 2) >> 1][addr];
  if (addr >= 0x180) {
    int b = sfr[0x25];
    if (b > 2) return 0xff;
...
void resetcpu() {
  ...
  parity[2] = 1;
  parity[3] = 0;
  for (i = 4; i < 16; i++) parity[i] = parity[i & 3] ^ parity[i >> 2];
  for (i = 16; i < 255; i++) parity[i] = parity[i & 15] ^ parity[i >> 4];
  sfr[0x06] = 0x7f;
  sfr[0x4c] = 0xff;
  sfr[0x01] = 0x02;
  t0h = t0l = 0;
...
int month_days() {
  ...
  if (m == 2) {
    ...
    return (m > 7 ? ((m & 1) ? 30 : 31) : ((m & 1) ? 31 : 30));
}

int handle_fwcall(int pc) {
  switch (pc) {
    case 0x100: {
      int i, a = ((ram[1][0x7d] << 16) | (ram[1][0x7e] << 8) | ram[1][0x7f]) &
                 0x1ffff;
      if (a >= gamesize)
        writemem(0x100, 0xff);
      else {
...
#ifdef __DC__
        if (!flash_written(a)) writemem(0x100, 0xff);
#endif
      }
      /*
    ...
    case 0x110: {
      ...
      int r = 0;
      for (i = 0; i < 0x80; i++)
        if ((r = (flash[(a & ~0xff) | ((a + i) & 0xff)] ^ ram[1][i + 0x80])) !=
            0)
          break;
      writemem(0x100, r);
    }
...
void run_cpu() {
  ...
  for (;;) {
    ...
    switch (i & 0xf) {
      ...
      case 1:
        switch (i >> 4) {
          ...
          case 9:
            ...
            writemem(0x101,
                     (readmem(0x101) & 0x3b) | (r + s + c > 255 ? 0x80 : 0) |
          ...
          case 0xb:
            ...
            c = (readmem(0x101) & 0x80) >> 7;
            writemem(0x100, r - s - c);
            writemem(0x101, (readmem(0x101) & 0x3b) |
                                (r - s - c < 0 ? 0x80 : 0) |
                                ((r & 15) - (s & 15) - c < 0 ? 0x40 : 0) |
                                (0 ? 4 : 0));
            break;
          case 0xc:
            cy = 2;
      ...
      case 6:
      case 7:
        r = readmem((i & 3) | ((readmem(0x101) >> 1) & 0xc)) |
            ((i & 2) ? 0x100 : 0);
        switch (i >> 4) {
          case 0:
            writemem(0x100, readmem(r));
      ...
      if (c0)
        if ((sfr[0x10] & 0xe0) == 0xe0) {
          ...
          if (sfr[0x10] & 0x40) {
            ...
            if (t0l >= 256) {
              t0l -= 256;
              if ((t0l += sfr[0x13]) >= 256) t0l = sfr[0x13];
              sfr[0x10] |= 2;
              if (sfr[0x10] & 1) intreq |= 1 << 2;
            }
          }
          if (sfr[0x10] & 0x80) {
            t0h += c0;
            if (t0h >= 256) {
              t0h -= 256;
              if ((t0h += sfr[0x15]) >= 256) t0h = sfr[0x15];
              sfr[0x10] |= 8;
              if (sfr[0x10] & 4) intreq |= 1 << 4;
            }
          }
    ...
    if (sfr[0x18] & 0xc0) {
      if ((sfr[0x18] & 0xe0) == 0xe0) {
        ...
        if (sfr[0x18] & 0x40) {
          ...
          if (t1l >= 256) {
            t1l -= 256;
            if ((t1l += sfr[0x1b]) >= 256) t1l = sfr[0x1b];
            sfr[0x18] |= 2;
            if (sfr[0x18] & 1) intreq |= 1 << 5;
          }
        }
        if (sfr[0x18] & 0x80) {
          t1h += cy;
          if (t1h >= 256) {
            t1h -= 256;
            if ((t1h += sfr[0x1d]) >= 256) t1h = sfr[0x1d];
            sfr[0x18] |= 8;
            if (sfr[0x18] & 4) intreq |= 1 << 5;
          }
        }
    ...
    if ((sfr[0x4e] & 3) == 3 && !imask) intreq |= 1 << 9;

    if (!intreq || imask || !(sfr[0x08] & 0x80)) continue;
    for (r = 0; r < 10; r++)
      if (intreq & (1 << r)) break;
    intreq &= ~(1 << r);
    push(pc & 0xff);
...
int do_vmsgame(char* filename, char* biosname) {
  if (filename == NULL && biosname == NULL) return 0;
  if (biosname != NULL)
    if (!loadbios(biosname)) return 0;
  if (filename != NULL)
    if (!loadflash(filename)) return 0;
  resetcpu();
  run_cpu();
  return 1;
}
