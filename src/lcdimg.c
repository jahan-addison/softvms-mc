...
static void display_frame(int f) {
  ...
  for (y = 0; y < h; y++) {
    unsigned char* p2 = p;
    for (x = 0; x < w; x++) vmputpixel(x, y, !!*p2++);
    p += lcdi_width;
  }
  redrawlcd();
}
...
static void free_framedata() {
  ...
  for (i = 0; i < lcdi_nframes; i++)
    if (lcdi_framedata[i] != NULL) free(lcdi_framedata[i]);
  free(lcdi_framedata);
}
...
int initimg(int fd) {
  ...
  int i, fsz;
  if (myread(fd, hdr, 16) < 16) return 0;
  if (strncmp(hdr, "LCDi", 4)) return 0;
  lcdi_width = hdr[6] + (hdr[7] << 8);
  lcdi_height = hdr[8] + (hdr[9] << 8);
  lcdi_nframes = hdr[14] + (hdr[15] << 8);
  fsz = lcdi_width * lcdi_height;
  if ((lcdi_frametime = calloc(lcdi_nframes, sizeof(long))) == NULL) return 0;
  for (i = 0; i < lcdi_nframes; i++) {
    if (myread(fd, tm, 4) < 4) {
      free(lcdi_frametime);
      return 0;
  ...
  for (i = 0; i < lcdi_nframes; i++) lcdi_framedata[i] = NULL;
  for (i = 0; i < lcdi_nframes; i++)
  ...
  if (lcdi_nframes > 1) {
    ...
    for (;;) {
      ...
      if (tnow.tv_sec > tnext.tv_sec ||
        ...
        if ((++fcntr >= lcdi_nframes)) fcntr = 0;
        display_frame(fcntr);
        tnext.tv_sec += lcdi_frametime[fcntr] / 1000;
        if ((tnext.tv_usec += (lcdi_frametime[fcntr] % 1000) * 1000) >=
            1000000) {
          tnext.tv_sec++;
