...
static union {
  unsigned char c[32 * 64 / 2];
  unsigned long l[32 * 64 / 8];
} mainimg;
...
static unsigned int icon_imagery[][22] = {
    {
        0x00092492, 0x00149249, 0x00149249, 0x000ffffe, 0x00080002, 0x0038fff2,
        0x00280002, 0x00280002, 0x00281ff2, 0x00280002, 0x00280002, 0x0028fff2,
        0x00280002, 0x00280002, 0x0029fff2, 0x00280002, 0x00280002, 0x002807f2,
        0x00280002, 0x002ffffe, 0x00200008, 0x003ffff8,
    },
    {
        0x0000ffc0, 0x00010020, 0x00020010, 0x00020c10, 0x00040c08, 0x00041e08,
        0x00041e08, 0x00043f08, 0x00043f08, 0x00047f88, 0x0004ffc8, 0x0004ffc8,
        0x0004ffc8, 0x00046d88, 0x00040c08, 0x00040c08, 0x00040c08, 0x00041e08,
        0x00020010, 0x00020010, 0x00010020, 0x0000ffc0,
    },
    {
        0x00001e00, 0x0000e1c0, 0x00030030, 0x00040c08, 0x00080c04, 0x00080c04,
        0x00100c02, 0x00100c02, 0x00100c02, 0x00200001, 0x00200c01, 0x00200c01,
        0x00200001, 0x00101802, 0x00103002, 0x00106002, 0x0008c004, 0x00088004,
        0x00040008, 0x00030030, 0x0000e1c0, 0x00001e00,
    },
    {
        0x00000800, 0x00000800, 0x00020c00, 0x00031e0e, 0x00039e7c, 0x0003dff8,
        0x0003fff0, 0x003fe3e0, 0x000fe3f8, 0x0003e3ff, 0x0007e3fc, 0x000fe3f0,
        0x001fe3c0, 0x003fffe0, 0x0001e3f0, 0x0001e3f8, 0x0001fffc, 0x0001df1e,
        0x00018f00, 0x00010700, 0x00000300, 0x00000100,
    },
};
...
static void* mkbgpixmap(char** vmu1_xpm, float* wret, float* hret, int* txret) {
  ...
  for (i = 1; i <= nc; i++) {
    ...
    (*pal)[0][*(unsigned char*)vmu1_xpm[i]] =
        (r << 16) | (g << 8) | (b) | 0xff000000;
  }
  if (txw > txh) {
    int bx;
    for (bx = 0; bx < w; bx += txh)
      twiddleit(vmu1_xpm + i, tex + bx * txh / 2, (bx + txh > w ? w - bx : txh),
                h, bx);
  } else {
    int by;
    for (by = 0; by < h; by += txw)
      twiddleit(vmu1_xpm + i + by, tex + by * txw / 2, w,
                (by + txw > h ? h - by : txw), 0);
  }
  *wret = 1.0 * w / txw;
  *hret = 1.0 * h / txh;
...
void texture_memcpy64(void* dest, void* src, int cnt) {
  ...
  unsigned int* d =
      (unsigned int*)(void*)(0xe0000000 | (((unsigned long)dest) & 0x03ffffc0));
...
#define GET_SXSY()                                                     \
  ...
  if (p < 1.0) {                                                       \
    sx = 0.25;                                                         \
    sy = 0.25;                                                         \
    dx = (640.0 / 4 - (BG_RIGHT - BG_LEFT) / 4) / 2 - 0.25 * BG_LEFT + \
         640.0 / 4 * (i & 3);                                          \
    dy = (480.0 / 2 - (BG_BOTTOM - BG_TOP) / 4) / 2 - 0.25 * BG_TOP +  \
         480.0 / 2 * (i >> 2);                                         \
    if (i == selected_vms)                                             \
      if (p == 0.0) {                                                  \
        sx *= wobble;                                                  \
...
static void draw_screen(float p) {
  ...
  if (p < 1.0 && (alpha = 0xff - 0x300 * p) > 0) {
    alpha2 = (alpha << 24) | (((alpha * 0xaa) >> 8) << 16) |
             (((alpha * 0xd5) >> 8) << 8) | ((alpha * 0xc3) >> 8);
  ...
  for (i = 0; i < VMSCNT; i++)
    if (i == selected_vms || (p < 1.0 && vmsexist[i] == 2)) {
      ...
      mypoly.cmd = TA_CMD_POLYGON | TA_CMD_POLYGON_TYPE_OPAQUE |
                   TA_CMD_POLYGON_SUBLIST | TA_CMD_POLYGON_STRIPLENGTH_2 |
                   TA_CMD_POLYGON_PACKED_COLOUR | TA_CMD_POLYGON_TEXTURED;
      ...
      mypoly.texture = TA_TEXTUREMODE_CLUT8 | TA_TEXTUREMODE_CLUTBANK8(0) |
                       ...
                       TA_TEXTUREMODE_ADDRESS(bg_texture);
  ...
  for (i = 0; i < VMSCNT; i++)
    if (i == selected_vms || (p < 1.0 && vmsexist[i] == 2)) {
      ...
      mypoly.cmd = TA_CMD_POLYGON | TA_CMD_POLYGON_TYPE_TRANSPARENT |
                   TA_CMD_POLYGON_SUBLIST | TA_CMD_POLYGON_STRIPLENGTH_2 |
                   TA_CMD_POLYGON_PACKED_COLOUR | TA_CMD_POLYGON_TEXTURED;
      mypoly.mode1 = TA_POLYMODE1_Z_GREATEREQUAL;
      mypoly.mode2 = TA_POLYMODE2_BLEND_SRC_ALPHA |
                     TA_POLYMODE2_BLEND_DST_INVALPHA |
                     TA_POLYMODE2_FOG_DISABLED | TA_POLYMODE2_TEXTURE_MODULATE |
                     TA_POLYMODE2_U_SIZE_32 | TA_POLYMODE2_V_SIZE_64 |
                     TA_POLYMODE2_ENABLE_ALPHA | TA_POLYMODE2_TEXTURE_CLAMP_U |
                     TA_POLYMODE2_TEXTURE_CLAMP_V | screen_filtering;
      mypoly.texture =
          TA_TEXTUREMODE_CLUT4 | TA_TEXTUREMODE_CLUTBANK4(16) |
          TA_TEXTUREMODE_TWIDDLED |
          TA_TEXTUREMODE_ADDRESS(
              screen_texture[i == selected_vms && p > 0.0 ? current_buffer
                                                          : i + 4]);
    ...
    for (i = 0; i < 4; i++)
      if (icon[i]) {
        ...
        mypoly.mode2 =
            ...
            TA_POLYMODE2_ENABLE_ALPHA | TA_POLYMODE2_TEXTURE_CLAMP_U |
            TA_POLYMODE2_TEXTURE_CLAMP_V | TA_POLYMODE2_BILINEAR_FILTER;
        mypoly.texture = TA_TEXTUREMODE_CLUT4 | TA_TEXTUREMODE_CLUTBANK4(16) |
                         TA_TEXTUREMODE_TWIDDLED |
                         TA_TEXTUREMODE_ADDRESS(icon_texture[i]);
...
void display_progress_bar(float p) {
  ...
  mypoly.cmd = TA_CMD_POLYGON | TA_CMD_POLYGON_TYPE_TRANSPARENT |
               TA_CMD_POLYGON_SUBLIST | TA_CMD_POLYGON_STRIPLENGTH_2 |
               TA_CMD_POLYGON_PACKED_COLOUR | TA_CMD_POLYGON_GOURAUD_SHADING;
...
int openmainwin() {
  ...
  for (i = 0; i < 1024; i++)
    twtab[i] = (i & 1) | ((i & 2) << 1) | ((i & 4) << 2) | ((i & 8) << 3) |
               ((i & 16) << 4) | ((i & 32) << 5) | ((i & 64) << 6) |
               ((i & 128) << 7) | ((i & 256) << 8) | ((i & 512) << 9);
  ...
  for (i = 0; i < 4 + VMSCNT; i++) {
    ...
    if (i >= 4) draw_id(i - 4);
    texture_memcpy64(screen_texture[i], &mainimg, sizeof(mainimg) / 64);
    memset(&mainimg, 0, sizeof(mainimg));
  }
  current_buffer = 0;
  icon[0] = icon[1] = icon[2] = icon[3] = 0;
  for (i = 0; i < 4; i++) icon_texture[i] = mkicontexture(icon_imagery[i]);
...
void checkevents() {
  ...
  for (pad = locked_get_pads(); i < 4; i++, pad++)
    if (pad->func & MAPLE_FUNC_CONTROLLER) {
      ...
      pressed &= ((buttons & 0xf0) >> 4) | ((buttons & 4) << 2) |
                 ((buttons & 2) << 4) | ((buttons & 0x600) >> 3);
  ...
  if (pressed != oldpressed) {
    ...
    for (i = 0; i < 8; i++)
      if ((bits >> i) & 1) keyrelease(i);
    bits = ~pressed & oldpressed;
    for (i = 0; i < 8; i++)
      if ((bits >> i) & 1) keypress(i);
    oldpressed = pressed;
  }
}
...
int gettimeofday(struct timeval* tp, struct timezone* tz) {
  ...
  if (tmlo < last_tm) tmhi++;

  unsigned long long usecs = ((((unsigned long long)tmlo) << 11) |
                              (((unsigned long long)tmhi) << 43)) /
                             100;
...
int selectvm() {
  ...
  for (;;) {
    int newkey, pressed;
    check_vmsavail();
    if (selected_vms >= 0 && vmsexist[selected_vms] != 2) selected_vms = -1;
    if (selected_vms < 0) {
      for (selected_vms = 0; selected_vms < VMSCNT; selected_vms++)
        if (vmsexist[selected_vms] == 2) break;
      if (selected_vms >= VMSCNT) selected_vms = -1;
    }
    checkevents();
    newkey = sfr[0x4c];
    pressed = oldkey & ~newkey;
    oldkey = newkey;
    if (pressed & (1 << 7)) return -1;
    if (pressed & (1 << 4)) break;
    if (selected_vms >= 0) {
