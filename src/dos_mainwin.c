...
BITMAP* xpmtobitmap(char** vmu1_xpm, PALETTE** pret) {
  ...
  for (y = 0; y < h; y++) {
    ...
    for (x = 0; x < w; x++) putpixel(bret, x, y, *off++);
  }
  *pret = p;
  return bret;
}
...
void fakebackground() {
  ...
  set_palette(*pal);
  clear(screen);
  blit(bm, screen, 0, 0, (SCREEN_W - bm->w) >> 1, (SCREEN_H - bm->h) >> 1,
       bm->w, bm->h);
  gFGColor = 'C';
  gBGColor = 'a';
}
...
BITMAP* genbackbitmap(PALETTE** pret) {
  ...
  switch (pixdbl) {
    ...
    case 3: /*CC- large lcd 144x80 display */
      ...
      bm_x = (SCREEN_W - bm->w) >> 1;
      bm_y = ((SCREEN_H - bm->h) >> 1) +
             20; /*CC- special hack so we can see DC logo! */
      off_x = bm_x + 50;
      off_y = bm_y + 130;
      gFGColor = 'C';
      gBGColor = 'a';
...
void vmputpixel(int x, int y, int p) {
  ...
  if (!screeninited) {
    if (!back_bm) back_bm = genbackbitmap(&back_pal);
    clear(screen);
...
void low_redrawlcd() {
  ...
  if (gDebugMode)
    textprintf_centre(screen, font, SCREEN_W / 2, 20, gFGColor, "Frame %d",
                      frame++);
}

void redrawlcd() { low_redrawlcd(); }
...
void checkevents() {
  ...
  if (gDebugMode)
    textprintf_centre(screen, font, SCREEN_W / 2, 0, gFGColor,
                      "KEYS: %d%d%d%d%d%d%d%d", old_up, old_dn, old_lt, old_rt,
                      old_a, old_b, old_s, old_m);
}
...
void waitforevents(struct timeval* t) {
  ...
  while (1) {
    gettimeofday(&now, NULL);
    if (now.tv_sec > dest.tv_sec) return;
    if (now.tv_sec == dest.tv_sec && now.tv_usec >= dest.tv_usec) return;
  }
}
