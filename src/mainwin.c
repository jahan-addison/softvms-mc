...
void init_sound() {
  static SDL_AudioSpec desired = {32768, AUDIO_S8,       1,   0, 256, 0,
                                  0,     sound_callback, NULL};
...
static Pixmap mkbgpixmap(Display* dpy, Window win, char** vmu1_xpm, int* wret,
                         int* hret) {
  ...
  if ((img = XCreateImage(dpy, attr.visual, attr.depth, ZPixmap, 0, data, w, h,
                          bpad, bpl)) == NULL) {
  ...
  for (y = 0; y < h; y++) {
    ...
    for (x = 0; x < w; x++) XPutPixel(img, x, y, pixel[*p++]);
  }
...
static XImage* mkmainimg(Display* dpy, Window win, int w, int h) {
  ...
  XGetWindowAttributes(dpy, win, &attr);
  if ((img = XCreateImage(dpy, attr.visual, 1, XYBitmap, 0, data, w, h, bpad,
                          bpl)) == NULL) {
    fprintf(stderr, "Failed to allocate XImage\n");
    free(data);
    return NULL;
...
Window openmainwin(Display* dpy) {
  ...
  if ((bgpm = mkbgpixmap(dpy, root, (pixdbl ? vmu_large : vmu_small), &w,
                         &h)) == None)
    return None;
  ...
  mainwin = XCreateWindow(
      dpy, root, 0, 0, w, h, 0, attr.depth, InputOutput, attr.visual,
      CWBackPixmap | CWBackingStore | CWColormap | CWBitGravity, &swa);
...
void checkevents() {
  ...
  if (QLength(maindpy) == 0) {
    ...
    if (select(x_fd + 1, &rfds, NULL, NULL, &t) > 0) {
      if (FD_ISSET(x_fd, &rfds)) XPeekEvent(maindpy, &event);
    }
  }
  while (QLength(maindpy) > 0) {
    ...
    switch (event.type) {
      case Expose:
        if (!event.xexpose.count) low_redrawlcd();
        break;
      case KeyPress:
...
void sound(int freq) { sound_freq = freq; }
