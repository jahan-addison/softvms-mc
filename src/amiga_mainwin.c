...
void __regargs _CXBRK(void) { exit(20); }
...
static BOOL open_ahi() {
  ...
  static struct AHISampleInfo info = {AHIST_M8S, sqr_wave, sizeof(sqr_wave)};
...
#endif
          (ahictrl =
               AHI_AllocAudio(AHIA_Channels, 1, AHIA_Sounds, 1, TAG_END)) &&
...
void sound(int freq) {
  ...
  AHI_Play(ahictrl, AHIP_BeginChannel, 0, AHIP_Freq, freq * 2, AHIP_Pan, 0x8000,
           AHIP_Vol, 0x10000, AHIP_Sound, 0, AHIP_EndChannel, 0, TAG_END);
...
#ifdef __amigaos4__
    if ((IKeymap =
             (struct KeymapIFace*)GetInterface(KeymapBase, "main", 1, 0))) {
#endif
      if ((TimePort = CreateMsgPort())) {
        if ((TimeReq = (struct timerequest*)CreateIORequest(
                 TimePort, sizeof(struct timerequest)))) {
          if (!OpenDevice(TIMERNAME, UNIT_MICROHZ, (struct IORequest*)TimeReq,
                          0)) {
...
DESTRUCTOR(mainwin) {
  ...
  if (GfxBase->LibNode.lib_Version >= 39) {
    if (myfgpen >= 0) ReleasePen(maindpy->ViewPort.ColorMap, myfgpen);
    if (mybgpen >= 0) ReleasePen(maindpy->ViewPort.ColorMap, mybgpen);
    for (i = 0; i < 256; i++)
      if (pixel[i] >= 0) ReleasePen(maindpy->ViewPort.ColorMap, pixel[i]);
  }
  if (bgimg != NULL) FreeBitMap(bgimg);
  if (mainimg != NULL) FreeVec(mainimg);
  if (mainwin != NULL) CloseWindow(mainwin);
  if (maindpy != NULL) UnlockPubScreen(NULL, maindpy);
  if (TimerBase) {
...
static struct BitMap* mkbgpixmap(struct Screen* dpy, char** vmu1_xpm, int* wret,
                                 int* hret) {
  int i, w, h, nc, x, y;
  struct BitMap* img;
  struct RastPort rp;
  ...
  for (i = 1; i <= nc; i++) {
    ...
    if (GfxBase->LibNode.lib_Version < 39)
      pixel[*(unsigned char*)vmu1_xpm[i]] = (r + g + b > 300 ? 2 : 1);
                  ...
                  ObtainBestPen(maindpy->ViewPort.ColorMap, 0x1010101UL * r,
                                0x1010101UL * g, 0x1010101UL * b, OBP_Precision,
                                PRECISION_ICON, TAG_END)) < 0) {
      fprintf(stderr, "Failed to allocate color %s\n", vmu1_xpm[i] + 4);
      return NULL;
    }
  }
  if ((img = AllocBitMap(w, h, dpy->RastPort.BitMap->Depth, 0,
                         dpy->RastPort.BitMap)) == NULL) {
...
static WORDBITS* mkmainimg(struct Screen* dpy, struct Window* win, int w,
                           int h) {
  WORDBITS* img;
...
struct Window* openmainwin(struct Screen* dpy) {
  ...
  maindpy = dpy;
  if ((bgimg = mkbgpixmap(dpy, (pixdbl ? vmu_large : vmu_small), &w, &h)) ==
      NULL)
    return NULL;
  mainwin = OpenWindowTags(
      NULL, WA_PubScreen, dpy, WA_InnerWidth, w, WA_InnerHeight, h,
      WA_GimmeZeroZero, TRUE, WA_SmartRefresh, TRUE, WA_Title, "SoftVMS",
      WA_DragBar, TRUE, WA_CloseGadget, TRUE, WA_DepthGadget, TRUE, WA_Activate,
      TRUE, WA_IDCMP, IDCMP_RAWKEY | IDCMP_CLOSEWINDOW, TAG_END);
  if (mainwin == NULL) return NULL;
  BltBitMapRastPort(bgimg, 0, 0, mainwin->RPort, 0, 0, w, h, 0xc0);
  if ((mainimg = mkmainimg(dpy, mainwin, (pixdbl ? 96 : 48),
                           (pixdbl ? 80 : 40))) == NULL)
    return NULL;
  ...
  if (GfxBase->LibNode.lib_Version >= 39) {
    if ((myfgpen = ObtainBestPen(maindpy->ViewPort.ColorMap, 0xaaaaaaaaUL,
                                 0xd5d5d5d5UL, 0xc3c3c3c3UL, OBP_Precision,
                                 PRECISION_GUI, TAG_END)) >= 0 &&
        (mybgpen = ObtainBestPen(maindpy->ViewPort.ColorMap, 0x08080808UL,
                                 0x10101010UL, 0x52525252UL, OBP_Precision,
                                 PRECISION_GUI, TAG_END)) >= 0) {
...
void redrawlcd() {
  if (pixdbl)
    BltTemplate((PLANEPTR)mainimg, 0, WORDSPERLINE * 2, mainwin->RPort, 33, 86,
                96, 80);
  else
    BltTemplate((PLANEPTR)mainimg, 0, WORDSPERLINE * 2, mainwin->RPort, 17, 43,
                48, 40);
}
...
void checkevents() {
  ...
  while ((msg = GetMsg(mainwin->UserPort))) {
    ...
    switch (event->Class) {
      ...
      case IDCMP_RAWKEY:
        ...
        if (1 == MapRawKey(&ie, &buffer, 1, 0)) switch (buffer) {
            case 'a':
            case 'A':
              ie.ie_Code = 0x20;
              break;
  ...
  if (cw || (SetSignal(0L, SIGBREAKF_CTRL_C) & SIGBREAKF_CTRL_C)) _CXBRK();
}

void waitforevents(struct timeval* t) {
  ULONG mask;
  if (t) {
    if (t->tv_secs == 0 && t->tv_micro == 0) return;
    TimeReq->tr_node.io_Command = TR_ADDREQUEST;
    TimeReq->tr_time = *t;
    SendIO((struct IORequest*)TimeReq);
  }
  mask = Wait((1UL << mainwin->UserPort->mp_SigBit) |
              (1UL << TimePort->mp_SigBit) | SIGBREAKF_CTRL_C);
  ...
  if (mask & SIGBREAKF_CTRL_C) _CXBRK();
}
...
void __stdargs sleep(unsigned t) { Delay(t * 50); }
#endif
